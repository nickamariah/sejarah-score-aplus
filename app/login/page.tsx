...
export default function LogMasuk() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Formatkan e-mel maya (cth: m4001@irags.edu)
      ...
      // 3. Tarik peranan (role) dari Firestore Database
      ...
      if (docSnap.exists()) {
        ...
        // Simpan data asas di LocalStorage supaya Portal Murid boleh baca dengan pantas
        ...
        // 4. HALA TUJU PINTAR (SMART ROUTING)
        if (userData.role === "murid") {
          router.push("/murid"); // Bawa ke Portal Pelajar
        } else {
          router.push("/guru");  // Bawa ke Makmal Penyelidikan (Guru/Admin)
        }
        
      } else {
        setRalat("Data pengguna tidak dijumpai di pangkalan data.");
      }
      
    } catch (error: any) {
      console.error("Ralat Log Masuk:", error);
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        setRalat("ID Pengguna atau Kata Laluan salah. Pastikan akaun ini telah didaftarkan.");
      } else {
        setRalat("Berlaku ralat. Sila pastikan maklumat anda betul.");
      }
    }
    setLoading(false);
  };
