...
function KandunganSijil() {
  ...
  // 🌟 LOGIK AUTO-RESIZE NAMA MURID (LEBIH KETAT)
  ...
  if (panjangNama > 40) {
    ...
    saizFontNama = "text-2xl md:text-4xl"; 
  }
  ...
  return (
    <div className="min-h-screen bg-slate-800 p-4 sm:p-8 flex flex-col items-center justify-center font-sans print:bg-white print:p-0">
      ...
      <div className="w-full overflow-x-auto flex justify-start md:justify-center pb-4 print:overflow-hidden print:pb-0">
        ...
        <div 
          className={`min-w-[900px] md:min-w-0 w-full max-w-4xl aspect-[1.414] bg-linear-to-br ${tema.gradient} rounded-lg shadow-2xl p-8 flex flex-col relative overflow-hidden shrink-0 mx-auto print:shadow-none print:w-full print:h-screen print:rounded-none print:p-12`}
        >
          {/* BINGKAI DALAMAN */}
          <div className={`absolute inset-4 md:inset-6 border-[6px] border-double ${tema.border} opacity-50 rounded-lg pointer-events-none print:inset-8`}></div>
          
          {/* BINTANG PENJURU */}
          <Star className={`absolute top-8 left-8 md:top-10 md:left-10 w-8 h-8 md:w-12 md:h-12 ${tema.border.replace('border-', 'text-')} opacity-30 print:top-14 print:left-14`} fill="currentColor" />
          <Star className={`absolute top-8 right-8 md:top-10 md:right-10 w-8 h-8 md:w-12 md:h-12 ${tema.border.replace('border-', 'text-')} opacity-30 print:top-14 print:right-14`} fill="currentColor" />
          <Star className={`absolute bottom-8 left-8 md:bottom-10 md:left-10 w-8 h-8 md:w-12 md:h-12 ${tema.border.replace('border-', 'text-')} opacity-30 print:bottom-14 print:left-14`} fill="currentColor" />
          <Star className={`absolute bottom-8 right-8 md:bottom-10 md:right-10 w-8 h-8 md:w-12 md:h-12 ${tema.border.replace('border-', 'text-')} opacity-30 print:bottom-14 print:right-14`} fill="currentColor" />

          {/* KANDUNGAN TENGAH (Diperkemas jarak margin supaya muat cantik) */}
          <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-4 md:px-12 w-full">
            
            <div className="mb-4">
              <Award className="w-16 h-16 md:w-24 md:h-24 text-yellow-500 drop-shadow-md mx-auto" />
            </div>

            <p className="text-xs md:text-sm font-bold tracking-[0.2em] text-slate-500 uppercase mb-2">I-RAGs Tutor • Sistem Pembelajaran Sejarah</p>
            <h1 className="text-4xl md:text-6xl font-extrabold text-slate-800 mb-6 drop-shadow-sm font-serif">
              SIJIL PENGUASAAN
            </h1>

            <p className="text-sm md:text-xl text-slate-600 mb-4 italic">Dengan bangganya dianugerahkan kepada:</p>
            
            {/* 🌟 NAMA MURID - Dikunci 1 baris menggunakan whitespace-nowrap dan overflow-hidden */}
            <div className="w-full px-4 mb-6">
               <h2 className={`${saizFontNama} font-black text-sky-900 border-b-2 border-slate-300 pb-2 inline-block px-4 md:px-8 uppercase tracking-wide whitespace-nowrap overflow-hidden text-ellipsis max-w-full`}>
                 {namaMurid}
               </h2>
            </div>

            <p className="text-sm md:text-lg text-slate-700 max-w-2xl leading-relaxed mb-6 px-4">
              ...
              dengan memperoleh skor ujian sebanyak <span className="font-bold text-xl md:text-2xl text-emerald-600">{skor}%</span>.
            </p>

            {/* 🌟 KATA-KATA SEMANGAT - Dinaikkan ke atas supaya tak tersangkut di bucu border */}
            <div className={`bg-white/60 p-4 md:p-5 rounded-2xl border ${tema.border} shadow-sm max-w-3xl w-full mx-auto`}>
