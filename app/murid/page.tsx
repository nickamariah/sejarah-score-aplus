...
export default function MuridDashboard() {
  ...
  const openModule = (chapterId: number, moduleId: number, aras: string) => {
    ...
    else if (moduleId === 2) window.location.href = `/pembelajaran?bab=tingkatan${t}_bab${chapterId}_sub1.1&aras=${aras}`;
  ...
  const getAdaptiveMeta = (chapterId: number, moduleId: number) => {
    ...
    if (skor === undefined) {
      ...
      // JIKA LULUS ATAU DAH SIAP POST TEST
  ...
  // 🌟 FUNGSI BARU: ANALISIS STATUS PENGUASAAN SETIAP BAB
  const getChapterStatus = (chapterId: number) => {
    ...
    if (skor === undefined) {
      return { label: "Belum Mula", color: "bg-slate-100 text-slate-500 border-slate-200", bar: "w-0", icon: "⚪" };
    }
    if (skor >= 80 || isPostTestSelesai) {
      return { label: "Kuasai", color: "bg-emerald-50 text-emerald-700 border-emerald-200", bar: "w-full bg-emerald-500", icon: "✅" };
    }
    if (isBimbinganSelesai) {
      return { label: "Sedia Ujian", color: "bg-sky-50 text-sky-700 border-sky-200", bar: "w-3/4 bg-sky-500", icon: "🚀" };
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-8 md:px-6 font-sans text-slate-900">
      <div className="mx-auto max-w-6xl">
        ...
        {/* 🌟 KOTAK ANALISIS PENGUASAAN BAB (FUNGSI BARU) */}
